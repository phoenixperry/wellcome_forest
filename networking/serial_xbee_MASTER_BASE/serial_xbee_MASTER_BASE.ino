...
void summerStormManager(){
  // If the hut is also in a winning state, but the treeTimer hasn't elapsed yet, run the tree part of the big win animation.
  if (hut_state==2 && tree_state==2 && (currentTime-treeTimer > TREE_WIN_DURATION) && (currentTime-hutTimer > HUT_WIN_DURATION)&& !summerStorm){
    summerStorm = 1;
    animationTimer = millis();  // set the animation timer

  // end the animation. Time is elapsed.
  }else if(summerStorm && (currentTime - animationTimer > GLOBAL_WIN_DURATION)){
    tree_state = 0;
    trees_current_beacon="C";
    hut_win = 0;
    summerStorm = 0;

  // just keep going. Mostly a placeholder. 
  }else{
    summerStorm = 1;
  }
}
